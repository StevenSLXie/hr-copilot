...
export const readDocx = async (fileUrl: string): Promise<TextItem[]> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (event) => {
      ...
      const textItems: TextItem[] = text.split('\n').map((line, index) => ({
        text: line,
        x: 0, // Default x coordinate
        y: index, // Use index as y coordinate
        width: line.length, // Default width based on line length
        height: 1, // Default height
        fontName: 'default', // Default font name
        hasEOL: line.endsWith('\n'), // Check if line ends with a newline character
      }));
