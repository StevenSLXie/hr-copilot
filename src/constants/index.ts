...
"3. Weakness: highlight 6-8 points that the candidate is not strong about and how can the resume be polished to address the weak points. give examples, be specific and critical and detailed; focus on content, do not comment on the formatting of resume;" + 
...
"5. Self-introduction: based on the resume content, prepare self-introduction for the candidate in 6-8 sentences, use first pronoun;" +
"6. Questions: list 5 questions you would ask if you are the hiring manager in order to deep dive into the candaiate's weakness. Must be specific. each question one line;" +
"7. Salary: based on the resume, guess the salary range of the candidate given his location. Propose the future salary range if the candidate is hired." + 
"the return should be in the format of Verdict: text; Summary: text; Weakness: text; Strength: text; Self-introduction: text; Questions: text; Salary: text; the index and the dot must be included. The overall response should be around 500 words";
...
"5. 面试问题：如果您是招聘经理，列出最重要的6个问题，以便深入了解候选人的弱点和具体的项目经验。必须具体。每个问题1-2行；" +
...
`Based on candaiate's resume, ask ${LIMITS.QUESTIONS_COUNT} questions to deep dive into the candidate's experience, competency, weakness and suitability.` +
...
"For each question, provide a guideline for candidate: do not directly answer it. Instead, hint on what the interviewer is looking for, tips on how to answer it, and example of what you should say in first few sentences." +
`the return should be in the format of Q: text; Guideline: text; A: text; Each question & guideline should be around 100 words. There must be ${LIMITS.QUESTIONS_COUNT} questions no matter what.`;

export const CMN_QUESTION_PROMPT = "您是一名招聘经理，根据以下候选人简历，对候选人进行面试。" +
`根据候选人的简历，提出${LIMITS.QUESTIONS_COUNT}个问题，以深入了解候选人的经验，能力，缺点和是否胜任相关岗位。` +
"问题必须具体，更多地关注候选人的过去项目经验。其中2-3个问题也可以是行为、性格方面的考察问题。对于技术角色，问题应围绕候选人的具体技能，例如编程语言" +
"对于每个问题，都需要提供一个答案，以帮助候选人准备面试。" +
`返回值的格式应如下 Q: text; A: text; 每个问题的答案应在100字左右。必须有${LIMITS.QUESTIONS_COUNT}个问题。用中文问答`;
